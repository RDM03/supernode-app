...
class _ExportMxcPreYearPageState extends State<ExportMxcPreYearPage> {
  ...
  void initState() {
    super.initState();
    final FiatCurrency fiatPreviousSession =
        context.read<StorageRepository>().selectedFiatForExport();
    context
        .read<SettingsCubit>()
        .initExportMxcPreYearPage(widget.year, fiatPreviousSession);
  }
              ...
              if (status.isGranted) {
                ...
                if (filePath.isNotEmpty) {
                  ...
                  Navigator.of(context).push(MaterialPageRoute(
                      builder: (context) => (filePath.isNotEmpty &&
                              context.read<SettingsCubit>().state.format ==
                                  "pdf")
                          ? PDFViewerPage(filePath: filePath)
                          : CsvViewerPage(filePath: filePath)));
                }
              }
            },
