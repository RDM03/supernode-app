...
class ScreenUtil {
  ...
  ScreenUtil init(double width, double height, BuildContext context) {
    ...
    try {
      ...
      _realSize = Size(data.size.width + data.padding.left + data.padding.right,
          data.size.height + data.padding.bottom + data.padding.top);
      _size = data.size;
      _physicalSize =
  ...
  double get density =>
      ScreenUtil.instance.boxHeight / ScreenUtil.instance.bluePrintHeight;
